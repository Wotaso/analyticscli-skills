...
function renderConnectorPicker(cursorIndex, selected, required, warning = '') {
...
async function askConnectorSelectionByKeys() {
    ...
    const selected = new Set(CONNECTOR_KEYS.filter((key) => required.has(key) || !isConnectorLocallyConfigured(key)));
    let warning = '';
...
async function getHealthCheckPlan(configPath, selected) {
    ...
    const hasAsc = selectedSet.has('asc') ||
    ...
    if (hasAnalytics)
        lines.push('AnalyticsCLI: token auth + readonly query');
    if (hasSentry)
        lines.push('Sentry / GlitchTip: token/org API + project discovery');
    if (hasRevenueCat)
        lines.push('RevenueCat: API key auth + project read');
    if (hasAsc)
        lines.push('ASC: API key auth + App Store Connect read');
    if (hasGitHub && githubRepo)
        lines.push(`GitHub: repo access (${githubRepo})`);
    if (hasGitHub && !githubRepo)
        lines.push('GitHub: skipped until repo is known');
...
async function guideSentryConnector(rl, secrets) {
    ...
    while (true) {
        ...
        const org = await ask(rl, `Sentry org slug for ${label} (leave empty to defer)`, index === 0 ? process.env.SENTRY_ORG || '' : '');
...
async function runConnectorSetupWizard(args) {
    ...
    try {
        let selected = withMissingRequiredAnalyticsConnector(args.connectors ? parseConnectorList(args.connectors) : await askConnectorSelection(rl));
        if (selected.length === 0) {
