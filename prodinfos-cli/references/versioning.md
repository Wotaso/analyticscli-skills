...
- `prodinfos-supported-range` is the CLI version range those instructions were written against.

These are different and should stay different.

## Recommended Policy

- Keep `prodinfos-cli` as the current stable skill.
- Update its supported range as long as the command surface stays compatible.
- If a future major CLI release breaks commands or workflows, create a new sibling skill such as `prodinfos-cli-v1`.
