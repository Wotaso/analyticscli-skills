...
| `PADDLE_API_KEY` | Optional Paddle Billing metrics refresh | Optional, ask before setup | Dedicated live API key for production revenue trends. If Paddle exposes key permissions, use metrics/revenue read access only; sandbox metrics are not production evidence |
