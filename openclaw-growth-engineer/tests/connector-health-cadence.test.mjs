...
test('ASC wizard requests the report-creation role and vendor number', () => {
  ...
  assert.match(wizard, /Recommended for normal continuous analytics downloads: Sales and Reports/);
  assert.match(wizard, /Create two App Store Connect API keys before continuing/);
  assert.match(wizard, /Steady-state key: Sales and Reports, or Finance/);
  assert.match(wizard, /Temporary bootstrap key: Admin/);
  assert.match(wizard, /deletes the local Admin \.p8 immediately after analytics initialization/);
  assert.match(wizard, /For the steady-state key, copy these values into this wizard/);
  assert.match(wizard, /Admin is not required for daily or continuous analytics ingestion/);
  assert.match(wizard, /one-time report-request bootstrap/);
  assert.match(wizard, /ASC_BOOTSTRAP_KEY_ID/);
  assert.match(wizard, /Temporary Admin bootstrap key required for ASC analytics setup/);
  assert.match(wizard, /ASC_BOOTSTRAP_KEY_ID \(temporary Admin key, required\)/);
  assert.match(wizard, /Growth Engineer setup requires a temporary Admin bootstrap key/);
  assert.match(wizard, /The wizard does not save these ASC_BOOTSTRAP_\* values to secrets\.env/);
  assert.match(wizard, /Deleted temporary Admin \.p8/);
  assert.match(wizard, /revoke the temporary Admin API key in App Store Connect/);
  ...
  assert.match(wizard, /Growth Engineer first checks for existing ongoing App Analytics report requests/);
  assert.match(wizard, /Daily ingestion does not create a new request every day/);
