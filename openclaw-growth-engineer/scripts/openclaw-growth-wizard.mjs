...
async function guideSentryConnector(rl, secrets) {
    ...
    while (true) {
        ...
        const tokenEnv = await ask(rl, `Token env var for ${label}`, defaultSentryTokenEnv({ index, label, baseUrl }));
        ...
        const projects = parseCommaList(await ask(rl, `Known Sentry projects for ${label} (comma-separated, leave empty to defer)`, ''));
        accounts.push({
            id,
            label,
            baseUrl,
...
async function runConnectorSetupWizard(args) {
    ...
    try {
        const selected = withMissingRequiredAnalyticsConnector(args.connectors ? parseConnectorList(args.connectors) : await askConnectorSelection(rl));
        if (selected.length === 0) {
