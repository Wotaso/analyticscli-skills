...
- If ASC web analytics is not logged in or the user-owned web session expired, tell the user exactly how to refresh it: set `ASC_WEB_APPLE_ID` to the Apple Account email, run `asc web auth login --apple-id "$ASC_WEB_APPLE_ID"`, verify with `asc web auth status --output json --pretty`, then rerun Growth Engineer. Do not confuse this with API-key ASC auth.
