...
For Paddle setup, the wizard must explain the account-level metrics model: do not ask for or hard-code one Paddle product/project. The user should create a dedicated live API key at `https://vendors.paddle.com/authentication`. If the Paddle UI offers API-key permissions, use only the permissions needed for metrics/revenue reads. If the UI does not expose scope selection, tell the user to use a dedicated Growth Engineer key that can be rotated/revoked.
